...
tools = toolkit.get_tools() + math_tools
...
conversation_history = [SystemMessage(content=system)]

# Create the agent
agent = create_react_agent(
    llm, 
    tools, 
    messages_modifier=None  # messages will be handles manually
)
...
while True:
    ...
    # Collect the outputs
    outputs = []
    for s in agent.stream({'messages': conversation_history}):
        outputs.append(s)
    ...
    conversation_history.append(final_output['agent']['messages'][0])
